...
watch(
  ...
  (visible) => {
    if (visible) {
      ...
      form.shares = 100;
      form.average_price =
        props.stock?.current_price || lastAveragePrice.value || 1;
...
function fillAllShares() {
  if (props.mode !== "sell" || !props.stock) return;
  const totalShares = Math.max(1, Math.round(props.stock.total_shares || 0));
  form.shares = totalShares;
  clearedOnFocus.shares = true;
}
  ...
  >
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
      ...
      >
        ...
        />
        <div
          v-if="mode === 'sell' && stock?.total_shares"
          class="field-actions"
        >
          <el-button text type="primary" @click="fillAllShares"
            >全部卖出</el-button
          >
          <span class="field-hint"
            >当前可卖：{{ Math.round(stock.total_shares) }} 股</span
          >
        </div>
      </el-form-item>
      <el-form-item
        :label="mode === 'buy' ? '买入价格' : '卖出价格'"
        prop="average_price"
      >
        <el-input-number
      ...
      <div class="inline-tip">
        本次{{ mode === "buy" ? "成交额" : "回收金额" }}：{{
          ((form.shares || 0) * (form.average_price || 0)).toFixed(2)
        }}
      </div>
    </el-form>
