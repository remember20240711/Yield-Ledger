...
const portfolioView = ref<"active" | "sold">("active");

const activeStocks = computed(() =>
  stocks.value.filter((stock) => stock.total_shares > 0),
);
const soldStocks = computed(() =>
  stocks.value.filter((stock) => stock.total_shares <= 0),
);
const displayedStocks = computed(() =>
  portfolioView.value === "active" ? activeStocks.value : soldStocks.value,
);
...
function setPortfolioView(view: "active" | "sold") {
  ...
  try {
    ...
    ElMessage.error(
      error?.response?.data?.detail ||
        `${payload.transaction_type === "buy" ? "买入" : "卖出"}失败`,
    );
  } finally {
    actionLoading.value = false;
  }
...
async function handleDeleteStock(stock: StockRow) {
  try {
    await ElMessageBox.confirm(
      `确认删除 ${stock.name}（${stock.normalized_symbol}）及其所有交易记录吗？`,
      "删除确认",
      {
        type: "warning",
...
function onDataToolCommand(command: string | number | object) {
  void handleDataToolCommand(command as DataToolCommand);
}
...
async function handleImportFileChange(event: Event) {
  ...
  try {
    ...
    await loadDashboard();
    ElMessage.success(
      `导入成功：${result.imported_stocks} 只股票 / ${result.imported_transactions} 条交易`,
    );
  } catch (error: any) {
    ElMessage.error(
      error?.response?.data?.detail || "导入失败，请检查文件内容",
    );
  } finally {
    backupLoading.value = false;
  }
...
<template>
  <div class="page-shell">
    <TopMenuBar
      :active-view="portfolioView"
      :active-count="activeStocks.length"
      :sold-count="soldStocks.length"
      @change-view="setPortfolioView"
    />
          ...
          >
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                >
                <el-dropdown-item command="export_json"
                  >导出备份 JSON</el-dropdown-item
                >
                <el-dropdown-item command="import_json"
                  >导入 JSON</el-dropdown-item
                >
              </el-dropdown-menu>
            </template>
          </el-dropdown>
